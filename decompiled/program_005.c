...
/* mean_absolute_deviation(std::vector<float, std::allocator<float> >) */

float mean_absolute_deviation(vector<float,std::allocator<float>> *param_1)

{
  ...
  local_14 = 0.0;
  for (local_24 = 0; uVar1 = std::vector<float,std::allocator<float>>::size(param_1),
      (ulong)(long)local_24 < uVar1; local_24 = local_24 + 1) {
    pfVar2 = (float *)std::vector<float,std::allocator<float>>::operator[](param_1,(long)local_24);
    local_14 = *pfVar2 + local_14;
  }
  ...
  for (local_24 = 0; uVar1 = std::vector<float,std::allocator<float>>::size(param_1),
      (ulong)(long)local_24 < uVar1; local_24 = local_24 + 1) {
    pfVar2 = (float *)std::vector<float,std::allocator<float>>::operator[](param_1,(long)local_24);
    fVar3 = (float)std::abs(*pfVar2 - local_14 / local_3c);
    local_1c = fVar3 + local_1c;
  }
  uVar1 = std::vector<float,std::allocator<float>>::size(param_1);
  local_54 = (float)uVar1;
  return local_1c / local_54;
}
