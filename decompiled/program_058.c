...
/* monotonic(std::vector<float, std::allocator<float> >) */

bool monotonic(vector<float,std::allocator<float>> *param_1)

{
  ...
  local_1c = 0;
  local_20 = 0;
  for (local_24 = 1; uVar2 = std::vector<float,std::allocator<float>>::size(param_1),
      (ulong)(long)local_24 < uVar2; local_24 = local_24 + 1) {
    pfVar3 = (float *)std::vector<float,std::allocator<float>>::operator[](param_1,(long)local_24);
    fVar1 = *pfVar3;
    pfVar3 = (float *)std::vector<float,std::allocator<float>>::operator[]
                                (param_1,(long)(local_24 + -1));
