...
/* has_close_elements(std::vector<float, std::allocator<float> >, float) */

undefined1 has_close_elements(float param_1,vector<float,std::allocator<float>> *param_2)

{
  ...
  do {
    ...
    while( true ) {
      ...
      ;
      std::abs(fVar1 - *pfVar3);
      if (extraout_XMM0_Da < param_1) {
        return 1;
      }
    }
    local_20 = local_20 + 1;
